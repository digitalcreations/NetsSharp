namespace NetsSharp
{
    ...
    public class RequestBuilder
    {
        public Uri GetUri<T>(Uri endpoint, IDictionary<string, string> arguments, T options = default(T))
        {
            ...
            if (options != null) { 
                var properties = typeof(T).GetRuntimeProperties().Where(p => p.CanRead)
                    .Where(p => (p.PropertyType != typeof(bool) && p.GetValue(options) != null) || (p.PropertyType == typeof(bool) && (bool)p.GetValue(options)))
                    .Select(p => new KeyValuePair<string, string>(String.Format("{0}{1}", p.Name.Substring(0, 1).ToLowerInvariant(), p.Name.Substring(1)), p.GetValue(options).ToString()))
                    .ToList();
        ...
        {
            return this.GetUri(endpoint, arguments, (object)null);
        }
    }
}